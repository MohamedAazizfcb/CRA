...
class ResidualParallelWaveGANDiscriminator(torch.nn.Module):
    """Parallel WaveGAN Discriminator module.
    - To solve when the input and output dimensions don’t match up
    - Batch normalization used after each conv and before activation 
    - Do not use dropout
    - skip connection "skip over" 2 layers
    """
    ...
    def __init__(self,
                 ...
                 out_channels=1,
                 kernel_size=3,
                 layers=30, #instead of 10
                 stacks=3,  #30/3=10
                 residual_channels=64,
                 gate_channels=128,   #2 layers to skip
                 skip_channels=64,     
                 dropout=0.0,
                 bias=True,
                 use_weight_norm=True,
    ...
    def forward(self, x):
        ...
        # apply final layers
        #add skips + ReLU + conv1d1x1 + ReLU + conv1d1x1
        x = skips
        for f in self.last_conv_layers:
            x = f(x)
        return x
